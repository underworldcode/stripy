...
stripy-docker-help(){
  ...
  stripy-docker-browse [port:-8899]
      ...
      python3 -m webbrowser -t "http://localhost:8899" # if you have a standard python3
  ...
  2) Run a script with the docker installation version of python3 and all relevant
     modules/data pre-installed.

  stripy-docker-sh commands

    stripy-docker-sh python3 extract_values.py
                                              # Runs the docker version of python3
                                              # on the local script
